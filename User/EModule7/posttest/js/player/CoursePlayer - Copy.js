...
CoursePlayer.prototype.onProgress = function (e) {
    ...
    for (var i = 0; i < elements.length; i++) {
        ...
        if (parseInt(elements[i].time) < parseInt(e.currentTarget.currentTime)) {
            ...
            else if (elements[i].type == "hbargraph") {
                var baranim = new BarAnimation(document.getElementById(elements[i].id));
                baranim.animate();
                elements.splice(index, 1);
            }
...
CoursePlayer.prototype.loadPlayList = function () {
    ...
    for (var slideIndex = 0; slideIndex < this.Slides.length ; slideIndex++) {
        ...
        html += '<div class="list-items" id="listitem' + slideIndex + '" onclick="onClickPlayList(' + slideIndex + ',this);" >' + slideName + '<span>' + slideDuration + '</span></div>';
    }
    $('#playlist').html(html);
